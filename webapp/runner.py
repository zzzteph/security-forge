...
def _run_scan(repo_id: int, scan_id: int) -> None:
    ...
    cmd = [PY, str(ROOT / "orchestrate.py"), "--repo", repo["url"],
           ...
           "--output-dir", str(DATA_ROOT), "--silent"]
    ...
    if (repo.get("context") or "").strip():
        env["SECFORGE_EXTRA_CONTEXT"] = repo["context"].strip()
    ...
    try:
        ...
        p.wait()
        if buf:
            db.append_scan_log(scan_id, "".join(buf))
        rc = p.returncode
    ...
    db.update_scan(scan_id, status=st, finished=db._now(), commit_sha=commit,
                   new_count=stats["new"], mitigated_count=stats["mitigated"],
                   total_count=stats["total"],
                   error=None if rc == 0 else f"orchestrator exit {rc}")
    db.append_scan_log(scan_id, f"\n[runner] done rc={rc}: {stats['new']} new, "
                       f"{stats['mitigated']} mitigated, {stats['total']} open.\n")
    _finalize_repo(repo_id, scan_id, st, commit)
